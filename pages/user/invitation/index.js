...
const Invitation = () => {
    ...
    const setupPassword = async (values) => {
        if(router.query.code){
            ...
            let respond = await postRequest(apiPaths.USER_SETUP_PASSWORD, obj);
            if (respond.status) {
                setRespondView(2);
            }else{
                setRespondView(3);
    ...
    useEffect(() => {
        const validateToken = async () => {
            if (router.query.code) {
                ...
                let respond = await postRequest(apiPaths.VERIFY_USER_TOKEN, values);
                if (respond.status) {
                    setRespondView(1);
                } else {
                    setRespondView(3);
                        ...
                        <div className="flex flex-column align-items-center justify-content-center min-h-screen min-w-screen overflow-hidden">
                            <div className="surface-card p-4 shadow-2 border-round w-full lg:w-4">
                                <div className="text-center mb-5">
                                    ...
                                    <div className="text-900 text-3xl font-medium mb-3">Setup User Password</div>
                                </div>
                                <div>
                                    <Formik
